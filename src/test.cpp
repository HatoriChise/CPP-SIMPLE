...
{
    boundaryInfo[0] = {X_MIN, INLET, {1.0f, 0.0f}, DIRICHLET, 300.0f, 0.0f};         // west
...
void printMassFluxField(const StructuredMesh &mesh, const MassFluxField &massFlux)
{
    ...
    for(int ii = 0; ii < ncx; ++ii)
    {
        fmt::print(" {:>8}", fmt::format("v_n[{}]", ii));
    }
    fmt::print(" |");
    for(int ii = 0; ii < ncx; ++ii)
    {
        fmt::print(" {:>8}", fmt::format("v_s[{}]", ii));
    }
    fmt::print("\n");
    fmt::print("  {:-^100}\n", ""); // 分隔线
    ...
    for(int jj = ncy - 1; jj >= 0; --jj)
    {
        ...
        for(int ii = 0; ii < ncx; ++ii)
        {
            float vn_face = massFlux(ii, jj).mN / (rho * dx);
            fmt::print(" {:>8.3f}", vn_face);
        }
        fmt::print(" |"); // 分隔符
        ...
        for(int ii = 0; ii < ncx; ++ii)
        {
            float vs_face = massFlux(ii, jj).mS / (rho * dx);
            fmt::print(" {:>8.3f}", vs_face);
        }
        fmt::print("\n");
    }
    fmt::print("  {:-^100}\n", ""); // 分隔线
...
bool test_mass_flux_checkerboard_pressure()
{
    ...
    massFlux.updateFluxes(mesh, velocity, props, BCbyRef, &pressure, &uEq, &vEq);
    ...
    massFluxNoRC.updateFluxes(mesh, velocity, props, BCbyRef, nullptr, nullptr, nullptr);
    ...
    printMassFluxField(mesh, massFlux);
